...
def test_performance_serial():
    try:
        ...
        event = 'start'
        print("================================= Getting nodes in cluster ==========================================")
        get_nodes_info()

        create_secret()
        create_sc()

        print("================================= STEP-1 Creating pvc in bulk ==========================================")
        create_pvc(path_pvc_yaml)

        print("================================= STEP-2 Creating deployments in bulk ==========================================")
        ...
        print("================================= STEP-3 Drain worker 1 %s =======================================" % node_to_drain)
        # drain worker 1
        event = 'drain worker 1'
        drain_node(node_to_drain)
        # Make worker 1 schedulable
        print("============================= STEP-4 uncordon worker 1 %s ====================================" % node_to_drain)
        ...
        # drain worker 2
        print("================================= STEP-5 Drain worker 2 %s ======================================" % node_to_drain)
        event = 'drain worker 2'
        drain_node(node_to_drain)

        # Create additional pvc and deployments
        print("================================= STEP-6 Creating additonal pvcs and deployments ================================")
        ...
        print("============================= STEP-7 uncordon worker 2 %s ====================================" % node_to_drain)
        uncorden_node(node_to_drain)

        # Delete some pods
        print("================================= STEP-8 Delete few pods ================================")
        event = 'delete pods'
        delete_random_pods()
        ...
        print("============================= STEP-9 reboot worker 1 %s ====================================" % node_to_reboot)
        event = 'reboot worker'
        reboot_flag = reboot_node(node_to_reboot)

        # wait before verifying pods
        print("========= Sleeping for a min before verification...")
        sleep(60)
        ...
        manager.delete_dep_bulk(path_dep_yaml)
        manager.delete_dep_bulk(path_additional_dep_yaml)

        verify_pod_deletion_bulk()

        print("================================= Deleting PVCs ================================")
        manager.delete_pvc_bulk(path_pvc_yaml)
        manager.delete_pvc_bulk(path_additional_pvc_yaml)
        verify_pvc_deletion_bulk()

        print("================================= Deleting SC and Secret ================================")
        if sc is not None and manager.check_if_deleted(2, sc.metadata.name, "SC") is False:
            manager.delete_sc(sc.metadata.name)
        if array_secret is not None and manager.check_if_deleted(2, array_secret.metadata.name, "Secret",
                                                                 namespace=array_secret.metadata.namespace) is False:
            manager.delete_secret(array_secret.metadata.name, array_secret.metadata.namespace)
...
def delete_random_pods(count=2):
    ...
    try:
        ...
        print("Deleting %s pods and will verify they come to running again." % count)
        for pod in pods_to_delete:
            manager.hpe_delete_pod_object_by_name(pod.metadata.name, namespace=pod.metadata.namespace)
    except Exception as e:
        print(("Exception in delete_random_pods() :: %s" % e))
        raise e
...
def uncorden_node(node_name):
    try:
        ...
        print("Node uncorden response is %s" % response)
    except Exception as e:
        print(("Exception in uncorden_node :: %s" % e))
        raise e
...
def corden_node(node_name):
    try:
        ...
        print("Node corden response is %s" % response)
    except Exception as e:
        print(("Exception in corden_node :: %s" % e))
        raise e
...
def drain_node(node_name):
    try:
        ...
        print("================  Draining node %s  ================\n" % node_name)
        response = manager.drain_node(node_name)
        print("Node drain response is %s" % response)
        ...
        print(("Exception in drain_node :: %s" % e))
        raise e
...
def get_nodes_info():
    ...
    try:
        ...
        print("Worker nodes in cluster are %s" % list(map_worker_nodes.keys()))
    except Exception as e:
        print("Exception in test_get_nodes_info :: %s" % e)
        raise e
...
def create_secret():
    ...
    global path_base_yaml
    try:
        yml = path_base_yaml + "secret.yml"
        secret = manager.create_secret(yml)
        assert secret is not None, "Secret is not created."
        #print("Secret %s is created successfully" % secret.metadata.name)
        ...
        print("Exception in test_create_secret :: %s" % e)
        raise e
    finally:
        pass
...
def create_sc():
    ...
    try:
        ...
        #print("Storgae Class %s is created successfully" % sc.metadata.name)
        ...
        print("Exception in test_create_sc :: %s" % e)
        raise e
...
def create_pvc(yml):
    ...
    try:
        ...
        print("PVCs created successfully %s" % list(pvc_map.keys()))
        ...
        print("Exception in test_create_pvc :: %s" % e)
        raise e
    finally:
        pass
...
def create_dep(yml):
    try:
        ...
        # Create deployments
        dep_map = manager.create_dep_bulk(yml)
        #print(dep_map)
        ...
        while all_pods_running_time < 30 * 60:
            ...
            if ready_deps == obj_list:  # all deps are ready
                ...
                for name in obj_list_to_be_checked:
                    ...
                    #obj = dep_map[name]
                    flag, obj = manager.check_status(5, name, 'deployment', None)
                    #print(obj)
                    ...
                    for condition in condition_list:
                        if condition.message.startswith('ReplicaSet "%s-' % obj.metadata.name) and \
                            ...
                            #print(condition.message.split()[1])
        ...
        print(f"======= All pods came to running in {str(datetime.timedelta(0, all_pods_running_time))} ")
        print("Deployments created successfully %s" % list(dep_map.keys()))

    except Exception as e:
        print("Exception in test_create_dep :: %s" % e)
        raise e
...
def populate_pod_list_from_replicaset():
    ...
    print("Sleeping for 1 min before refreshing pod list...")
    sleep(60)
    print("Over from sleeping...")

    # Get pods in default namespace and match name start with replica set to get corresponding pod.
    list_all_pods = manager.hpe_list_pod_objects_names(namespace='default')
    # print("All pods in default namespace %s" % list_all_pods)

    # Get pod names
    list_pod_name.clear()
    for replica_set in list_replica_set:
        # print([i for i in list_all_pods if i.startswith(replica_set)])
...
def timer():
    ...
    # print("############ time has time :: %s " % all_pods_running_time)
...
def check_pod_status_vlun_crd(deploy_added=False, pod_removed=False):
    ...
    try:
        print("Verifying all pods in running state")
        list_pod_obj.clear()
        ...
        # obj_list = {pod_obj for pod_obj in dep_map.values()}
        while all_pods_running_time < 30 * 60:
            # print(f"ready_deps :: {ready_replicas}")
            ...
            if ready_replicas == replica_list:  # all deps are ready
                ...
                for replica_set_name in replica_list_to_be_checked:
                    ...
                    print("%s has %s list of pods" % (replica_set_name, pods_for_dep))
                    ...
                    """assert replica_has_running_pod is True, "Deployment %s does not have any pod in running state yet out of %s" % \
                                                            (replica_set_name[0:replica_set_name.index('-')],
                                                             pods_for_dep)"""
                    print("Deployment %s has pod in running state" % replica_set_name[0:replica_set_name.index('-')])
        assert pod_status_check_done is True, f"All pods did not come to running in " \
                                              f"{str(datetime.timedelta(0, all_pods_running_time))}, terminating test..."
        print("==================================== Time taken to all pods come to running is %s" % str(
            datetime.timedelta(0, all_pods_running_time)))
        print("Node wide distribution of pods...")
        for node, pod_list in map_pod_node_dist.items():
            print(f"{node} :: {pod_list}\n")

        print("Now verifying vlun and CRDs for each pod")
        ...
        for pod_obj in list_pod_obj:
            ...
            print("\n\nPVC is :: %s " % pvc_name)
            volume_name = manager.hpe_read_pvc_object(pvc_name).spec.volume_name
            print("volume_name is :: %s " % volume_name)
            assert manager.verify_pvc_crd_published(volume_name) is True, \
                "PVC CRD %s Published is false after Pod is running" % volume_name
            print("PVC CRD %s published is True" % volume_name)
            ...
            print("Node for pod received from 3par and cluster match")
            ...
            # print("%s is mounted on %s" % (pod_obj.metadata.name, pod_node_name))
            map_pod_node_dist[pod_node_name].append(pod_obj.metadata.name)

        print("\n\nSuccessfully verified vlun and CRD status for each pod")
        # print("Node wide distribution of pod %s" % map_pod_node_dist)
    except Exception as e:
        raise e
    finally:
        pod_status_check_done = True
...
def verify_vlun_cleanup(node_name=None):
    ...
    global list_pod_obj
    global access_protocol
    print("Now verifying node %s does not have any partitions and multipath entries from previous pods" % node_name)
    if access_protocol == 'iscsi':
        for pod_obj in list_pod_obj:
            pod_node = None
...
def cleanup_deployments():
    try:
        manager.delete_dep_bulk('ymls/dep')
        manager.delete_dep_bulk('ymls/additional_deps')
    except Exception as e:
        print(f"Exception in deleting deployments {e}")