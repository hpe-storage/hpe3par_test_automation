...
def test_virtual_copyOf_tpvv_vol():
    base_yml = 'YAML/virtual-copy/virtual-copy-base-vol.yml'
    snap_yml = 'YAML/virtual-copy/virtual-copy-snap-vol.yml'
    hpe3par_cli = None
    snap_pvc = None
    clone_pod_obj = None
    timeout = 900
    ...
    pdb.set_trace()
    try:
        with open('YAML/virtual-copy/testcase1_base_sc.yml', "r") as ymlfile:
        ...
        try:
            ...
            print("Checking snapshot volume data")
            ...
            print("snapshot volume data check successfull")
        except Exception as e:
            cleanup(None, sc_snap, pvc_snap, snap_pod_obj)
            cleanup(secret, sc, pvc, base_pod)
        time.sleep(10)
        ...
        print("Volume crd check successfull")
        ...
        isValid = manager.verify_volume_properties_primera(base_volume , cpg = cfg['parameters']['cpg'], provisioning = 'thin')
        ...
        assert isValid is True, "Validation of snapshot volume failed"
        print("volume check on the array successfull")
        time.sleep(10)
