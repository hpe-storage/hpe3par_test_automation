...
class TestK8S(KubernetesAutomationTest):
    ...
    def read_config(self):
        ...
        try:
            ...
            HPE3PAR_API_URL = str(config.get('DEFAULT', 'hpe3par_api_url'))
            HPE3PAR_IP = str(config.get('DEFAULT', 'san_ip'))
            HPE3PAR_USERNAME = str(config.get('DEFAULT', 'hpe3par_username'))
            HPE3PAR_PWD = str(config.get('DEFAULT', 'hpe3par_password'))
            ISCSI_IP = str(config.get('DEFAULT', 'hpe3par_iscsi_ips')).split(",")
    ...
    def test_create_objects(self,read_config):
        print("\n########################### test_create_objects ###########################")
        try:
            # Create SC, PVC and Pod
            pdb.set_trace()
            yml = "YAML/snapsched.yml"
            global sc_name, pvc_name, pod_name, pod_node_name, dep_name
            with open(yml) as f:
                elements = list(yaml.safe_load_all(f))
                for el in elements:
                    #print("======== kind :: %s " % str(el.get('kind')))
                    if str(el.get('kind')) == "StorageClass":
                        #print("StorageClass YAML :: %s" % el)
                        print("\nCreating SC...")
                        sc_name = self.hpe_create_sc_object(el).metadata.name
                        print("\nStorageClass %s created." % sc_name)
                    if str(el.get('kind')) == "PersistentVolumeClaim":
                        #assert self.check_status(timeout, sc_name, kind='sc',
                         #                        status='Available') == True, "Storage Clas status check timed out"
                    ...
                    if str(el.get('kind')) == "Deployment":
                        ...
                        #print("Pod YAML :: %s" % el)
                        print("\nCreating Deployment...")
                        dep_obj = self.hpe_create_deployment(yml)
                        dep_name = dep_obj.metadata.name
                        dep_created_obj = self.hpe_read_deployment(yml)
                        dep_status = self.hpe_read_deployment_status(dep_name)
                        assert self.check_status(timeout, dep_name, kind='deployment',
                                                 status=el['spec']['replicas']) == True, "Deployment status check timed out"
                        #import pytest;
                        #pytest.set_trace()
    ...
    def test_verify_cluster_vlun(self):
        ...
        global backend_volume_name, pod_name, node_name, vv_wwn
    ...
    def test_verify_delete_objects(self):
        ...
        try:
            ...
            global sc_name, pvc_name, pod_name, pod_node_name, dep_name
            #dep_obj = self.hpe_read_deployment(yml)
            # delete pod first
            print("Deleting Deployment...")
            deploy_delete_resp = self.hpe_delete_deploymente(yml)
            # print("Pod deletion response.  ::  %s " % pod_delete_resp)
            if self.check_if_deleted(timeout, pod_name, "pod"):
                assert dep_name not in self.hpe_list_deployment_objects_names(), "deployment %s is not deleted yet " % dep_name

            print("\ndeployment is deleted now, lets delete pvc...")
            print("Deleting PV...")
            pvc_resp = self.hpe_read_pvc_object(pvc_name)
            vol_name = pvc_resp.spec.volume_name
            ...
            print("\npvc is deleted now, lets delete sc...")
            print("Deleting SC...")
            #import pytest;
            #pytest.set_trace()
            sc_delete_resp = self.hpe_delete_sc_object(yml)
            #print("PV deletion response.  ::  %s " % pv_delete_resp)
            if self.check_if_deleted(timeout, sc_name, "sc"):
                assert sc_name not in self.hpe_list_sc_objects_names(), "sc %s is not deleted yet " % sc_name

            print("\npv is deleted now, lets delete volume...")
            print("Deleting docker volume...")
            if self.check_if_deleted(timeout, vol_name, "volume"):
                assert vol_name not in self.get_docker_client().volumes(), "volume %s is not deleted yet " % vol_name

            #import pytest;
            #pytest.set_trace()
            #self.get_docker_client().remove_volume(vol_name, force=False)
