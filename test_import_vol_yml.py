...
class TestK8S(KubernetesAutomationTest):
    ...
    def read_config(self):
        global HPE3PAR_API_URL, HPE3PAR_IP, ISCSI_IP, HPE3PAR_USERNAME, HPE3PAR_PWD, HPE3PAR_CPG, logger
        try:
            ...
            HPE3PAR_CPG = str(config.get('3par1', 'hpe3par_cpg'))
            ...
            print("HPE3PAR_CPG :: %s " % HPE3PAR_CPG)
    ...
    def test_create_objects(self,read_config):
        ...
        try:
            ...
            pdb.set_trace()
            yml = "YAML/import.yml"
            global pv_name, pvc_name, pod_name, pod_node_name, volume_name
            with open(yml) as f:
                elements = list(yaml.safe_load_all(f))
                for el in elements:
                    #print("======== kind :: %s " % str(el.get('kind')))
                    if str(el.get('kind')) == "PersistentVolume":
                        volume_name = el['spec']['flexVolume']['options']['name']
    ...
    def test_verify_delete_objects(self):
        ...
        try:
            ...
            #import pytest;
            #pytest.set_trace()
            self.get_docker_client().remove_volume(volume_name, force=False)
